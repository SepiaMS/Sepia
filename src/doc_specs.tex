...
Elles évoluent selon les résultats des mini-jeux et peuvent être temporairement altérées par certains objets (les \nameref{subsubsec:potions}). Ces augmentations ou diminutions ont, en retour, une conséquence directe sur le niveau de difficulté des épreuves.

Elles sont ici représentées sommairement. Pour une vision détaillée, voir la section \nameref{subsec:stats-detail}.

\subsubsection{Monnaie virtuelle}

La monnaie virtuelle est obtenue par les mini-jeux et permet l’acquisition d’objets virtuels, en visitant le \nameref{subsec:magasin}.
...
\begin{itemize}
    \item \nameref{subsec:quetes};
    \item \nameref{subsec:inventaire}~;
    \item \nameref{subsec:magasin}~;
    \item \nameref{subsec:stats-detail}.
\end{itemize}
...
Récompenses pratiques donnant au patient plusieurs recettes culinaires de plats équilibrés pour une meilleure hygiène alimentaire. % TODO Section à développer
